...
function FlowEditor({ agents, onSave }: { agents: AgentItem[]; onSave: (agent: AgentItem, steps: FlowStep[]) => Promise<void> }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="min-h-[510px] overflow-x-auto rounded-2xl border border-blue-400/15 bg-[#010b19]/70 p-8"><div className="flex min-w-max items-center py-28">
      {steps.map((step, index) => <div key={step.id} className="flex items-center" onDragOver={(e) => e.preventDefault()} onDrop={() => drop(index)}>
        {index > 0 && <div className="relative h-0.5 w-20 bg-cyan-500"><span className="absolute -right-1 -top-1.5 h-3 w-3 rounded-full border-2 border-cyan-500 bg-[#010b19]" /></div>}
